...
export const deploySiO2Libraries = async (
  verify?: boolean
...
export const deployLendingPool = async (verify?: boolean) => {
  const libraries = await deploySiO2Libraries(verify);
...
export const deploySiO2FallbackOracle = async (verify?: boolean) =>
  withSaveAndVerify(
    await new SiO2FallbackOracleFactory(await getFirstSigner()).deploy(),
    eContractid.SiO2FallbackOracle,
    [],
    verify
  );
...
export const deployMockedSiO2IncentivesController = async (
  args: [tEthereumAddress],
  verify?: boolean
) =>
  withSaveAndVerify(
    await new MockedSiO2IncentivesControllerFactory(await getFirstSigner()).deploy(args[0]),
    eContractid.MockedSiO2IncentivesController,
    args,
    verify
  );

export const deploySiO2Oracle = async (
  args: [tEthereumAddress, tEthereumAddress, string, string],
  verify?: boolean
) =>
  withSaveAndVerify(
    await new SiO2OracleFactory(await getFirstSigner()).deploy(args[0], args[1], args[2], args[3]),
    eContractid.SiO2Oracle,
    args,
    verify
  );
...
export const deployRedStoneTransparentUpgradeableProxy = async (
  args: [string, string, BytesLike],
  verify?: boolean
  ) =>
  withSaveAndVerify(
    await new RedStoneTransparentUpgradeableProxyFactory(await getFirstSigner()).deploy(args[0], args[1], args[2]),
    eContractid.RedStoneTransparentUpgradeableProxy,
    [args[0], args[1], args[2] as string],
    verify
  );
...
export const deploySiO2ProtocolDataProvider = async (
  addressesProvider: tEthereumAddress,
  verify?: boolean
) =>
  withSaveAndVerify(
    await new SiO2ProtocolDataProviderFactory(await getFirstSigner()).deploy(addressesProvider),
    eContractid.SiO2ProtocolDataProvider,
    [addressesProvider],
    verify
  );
...
export const deployGenericSToken = async (
  ...
  const instance = await withSaveAndVerify(
    await new STokenFactory(await getFirstSigner()).deploy(),
    eContractid.SToken,
    [],
    verify
  );
...
export const deployGenericSTokenImpl = async (verify: boolean) =>
  withSaveAndVerify(
    await new STokenFactory(await getFirstSigner()).deploy(),
    eContractid.SToken,
    [],
...
export const deployDelegationAwareSToken = async (
  ...
  const instance = await withSaveAndVerify(
    await new DelegationAwareSTokenFactory(await getFirstSigner()).deploy(),
    eContractid.DelegationAwareSToken,
    [],
    verify
  );
...
export const deployDelegationAwareSTokenImpl = async (verify: boolean) =>
  withSaveAndVerify(
    await new DelegationAwareSTokenFactory(await getFirstSigner()).deploy(),
    eContractid.DelegationAwareSToken,
    [],
...
export const deployAllMockTokens = async (verify?: boolean) => {
  ...
  const protoConfigData = getReservesConfigByPool(SiO2Pools.proto);
...
export const deploySTokensAndRatesHelper = async (
  args: [tEthereumAddress, tEthereumAddress, tEthereumAddress],
  verify?: boolean
) =>
  withSaveAndVerify(
    await new STokensAndRatesHelperFactory(await getFirstSigner()).deploy(...args),
    eContractid.STokensAndRatesHelper,
    args,
    verify
  );
...
export const deployMockSToken = async (
  args: [
    tEthereumAddress,
    tEthereumAddress,
    tEthereumAddress,
  ...
  const instance = await withSaveAndVerify(
    await new MockSTokenFactory(await getFirstSigner()).deploy(),
    eContractid.MockSToken,
    [],
    verify
  );
...
export const chooseSTokenDeployment = (id: eContractid) => {
  switch (id) {
    case eContractid.SToken:
      return deployGenericSTokenImpl;
    case eContractid.DelegationAwareSToken:
      return deployDelegationAwareSTokenImpl;
    default:
      throw Error(`Missing sToken implementation deployment script for: ${id}`);
  }
};

export const deploySTokenImplementations = async (
  pool: ConfigNames,
  ...
  // Obtain the different SToken implementations of all reserves inside the Market config
  const STokenImplementations = [
    ...Object.entries(reservesConfig).reduce<Set<eContractid>>((acc, [, entry]) => {
      acc.add(entry.STokenImpl);
      return acc;
    }, new Set<eContractid>()),
  ];

  for (let x = 0; x < STokenImplementations.length; x++) {
    const STokenAddress = getOptionalParamAddressPerNetwork(
      poolConfig[STokenImplementations[x].toString()],
      network
    );
    if (!notFalsyOrZeroAddress(STokenAddress)) {
      const deployImplementationMethod = chooseSTokenDeployment(STokenImplementations[x]);
      console.log(`Deploying implementation`, STokenImplementations[x]);
...
export const deployPriceAggregatorAdapterRedStoneImpl = async (args: [string], verify?: boolean) =>
  withSaveAndVerify(
    await new PriceAggregatorAdapterRedStoneImplFactory(await getFirstSigner()).deploy(args[0]),
    eContractid.PriceAggregatorAdapterRedStoneImpl,
    args,
    verify
  );
...
export const deployStakeUIHelper = async (
  [priceOracle, sio2, stakedSiO2, mockUsd]: [
    tEthereumAddress,
    tEthereumAddress,
    tEthereumAddress,
    tEthereumAddress
  ...
  const args = [priceOracle, sio2, stakedSiO2, mockUsd];
  return withSaveAndVerify(
...
export const deployFlashLiquidationAdapter = async (
  [receiver, addressesProvider, uniswapRouter]: [
    tEthereumAddress,
    tEthereumAddress,
    tEthereumAddress
  ],
  ...
  const args = [receiver, addressesProvider, uniswapRouter];
  return withSaveAndVerify(
