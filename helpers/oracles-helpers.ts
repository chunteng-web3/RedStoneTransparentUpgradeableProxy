import { SiO2FallbackOracle } from '../types';
...
export const setInitialMarketRatesInRatesOracleByHelper = async (
  ...
  for (let [assetSymbol, { borrowRate }] of Object.entries(marketRates) as [
    string,
    IMarketRates
  ][]) {
    if (assetSymbol === 'DOT') {
...
export const setInitialAssetPricesInOracle = async (
  ...
  for (let [assetSymbol, price] of Object.entries(prices) as [string, string][]) {
    try {
      if (assetSymbol === 'DOT') {
        assetSymbol = 'WDOT';
      }
      ...
      console.log(assetSymbol, assetsAddresses);
    }
  }
};
...
export const setAssetPricesInFallbackOracle = async (
  prices: iAssetBase<tEthereumAddress>,
  assetsAddresses: iAssetBase<tEthereumAddress>,
  priceOracleInstance: SiO2FallbackOracle
) => {
