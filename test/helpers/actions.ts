...
export const borrow = async (
  ...
  if (expectedResult === 'success') {
   ...
   var tx = await pool.connect(user.signer).borrow(reserve, amountToBorrow, interestRateMode, '0', onBehalfOf)
   for(const evt in (await tx.wait()).events){
...
const getDataBeforeAction = async (
  ...
  const STokenInstance = await getSToken(reserveData.STokenAddress);
  return {
    reserve,
    reserveData,
    userData,
...
export const getTxCostAndTimestamp = async (tx: ContractReceipt) => {
  ...
  if (txInfo.gasPrice === undefined) {
    throw new Error('No tx gas price');
  }
