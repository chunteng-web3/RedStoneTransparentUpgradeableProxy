...
export const calcExpectedUserDataAfterDeposit = (
  ...
  expectedUserData.scaledSTokenBalance = calcExpectedScaledSTokenBalance(
    userDataBeforeAction,
    reserveDataAfterAction.liquidityIndex,
    new BigNumber(amountDeposited),
    new BigNumber(0)
  );
  expectedUserData.currentSTokenBalance = calcExpectedSTokenBalance(
    reserveDataBeforeAction,
    userDataBeforeAction,
    txTimestamp
  ).plus(amountDeposited);

  if (userDataBeforeAction.currentSTokenBalance.eq(0)) {
...
export const calcExpectedUserDataAfterWithdraw = (
  ...
  const STokenBalance = calcExpectedSTokenBalance(
    reserveDataBeforeAction,
    userDataBeforeAction,
    txTimestamp
  );
  ...
  if (amountWithdrawn == MAX_UINT_AMOUNT) {
    amountWithdrawn = STokenBalance.toFixed(0);
  }
  ...
  expectedUserData.currentSTokenBalance = STokenBalance.minus(amountWithdrawn);
  ...
  if (userDataBeforeAction.currentSTokenBalance.eq(0)) {
    ...
    if (expectedUserData.currentSTokenBalance.eq(0)) {
...
export const calcExpectedReserveDataAfterWithdraw = (
  ...
  if (amountWithdrawn == MAX_UINT_AMOUNT) {
    amountWithdrawn = calcExpectedSTokenBalance(
      reserveDataBeforeAction,
      userDataBeforeAction,
      txTimestamp
    ).toFixed();
...
export const calcExpectedUserDataAfterBorrow = (
  ...
  expectedUserData.scaledSTokenBalance = userDataBeforeAction.scaledSTokenBalance;
...
const calcExpectedScaledSTokenBalance = (
  userDataBeforeAction: UserReserveData,
  index: BigNumber,
  amountAdded: BigNumber,
  amountTaken: BigNumber
) => {
  return userDataBeforeAction.scaledSTokenBalance
    .plus(amountAdded.rayDiv(index))
    .minus(amountTaken.rayDiv(index));
};

export const calcExpectedSTokenBalance = (
  ...
  const { scaledSTokenBalance: scaledBalanceBeforeAction } = userData;
